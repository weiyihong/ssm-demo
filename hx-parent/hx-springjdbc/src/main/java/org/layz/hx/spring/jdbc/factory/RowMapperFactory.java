...
public class RowMapperFactory {
	...
	public<T> RowMapper<T> getRowMapper(final Class<T> clazz){
		if(null == clazz) {
			throw new HxJdbcRuntimeException("class is null");
		}
		...
		if(null == tableClassInfo) {
			throw new HxJdbcRuntimeException("tableClassInfo is null");
		}
		final List<FieldColumnInfo> fieldList = tableClassInfo.getFieldList();
		if(null == fieldList || fieldList.isEmpty()) {
			throw new HxJdbcRuntimeException("fieldList is empty");
		}
