...
public class BaseJobDaoImpl<T extends BaseJobEntity> extends BaseDaoImpl<T> implements BaseJobDao<T>{
    ...
    public int updateNextTask(Long id) {
        LOGGER.debug("id: {}", id);
        String sql = "update " + tableName + " set `status` = ?,lastModifiedDate = ? where id = ? and `status` = ?";
        return getJdbcTemplate().update(sql, JobStatusEnum.WAITE_START.getValue(), new Date(), id,JobStatusEnum.WAITE_HANDLE.getValue());
    }
