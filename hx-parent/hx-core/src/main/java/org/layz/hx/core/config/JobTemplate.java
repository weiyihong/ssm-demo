package org.layz.hx.core.config;
    ...
    private void excuteSingleJob() {
        ...
        try {
            ...
            for (T entity : processList) {
                ...
                try {
                    // 获取具体要处理的业务类
                    taskServiceName = entity.getTaskService();
                    ...
                    LOGGER.error("run error, serviceName", entity.getTaskService(), e);
            ...
            if(null != nextTaskList && !nextTaskList.isEmpty()) {
                baseJobDelegate.updateNextTask(nextTaskList);
            }
        } catch (Throwable e) {
            LOGGER.error("#### systemTask handle error [{}] ####", scanTypeName, e);
        }
